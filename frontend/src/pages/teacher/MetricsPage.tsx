...
function typeLabel(t: string): string {
  if (t === "execution_trace" || t === "execution-trace") return "Execution Trace";
...
export function MetricsPage() {
  ...
  if (!data) {
    return (
      <Card padding="lg" className="text-center py-12">
        ...
        <p className="mt-3 text-sm font-medium text-stone-500">Unable to load metrics</p>
      </Card>
    );
  }

  const { totals, type_overview, ai_pass_distribution, theme_overview, telemetry } = data;
  const maxTypeItems = Math.max(...type_overview.map((t) => t.total_items), 1);
  const maxBucketTotal = Math.max(
    ...ai_pass_distribution.map((b) => b.parsons + b.dropdown + b.execution_trace),
    1
  );
  const maxThemeItems = Math.max(...theme_overview.map((t) => t.total_items), 1);
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="font-serif text-display-sm text-stone-900">Learning Analytics</h1>
        <p className="mt-1 text-stone-500">
          Assessment quality, AI difficulty distribution, and student interaction signals.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 xl:grid-cols-4">
        <StatBox
          ...
          helper={`${totals.evaluated_items} evaluated`}
          icon={Database}
        />
        <StatBox
          label="Low-AI Items"
          value={String(totals.low_ai_items)}
          helper={`≤50% AI pass rate`}
          icon={ShieldCheck}
        />
        <StatBox
          label="Avg AI Pass"
          value={formatPercent(totals.avg_ai_pass_rate)}
          helper={`${totals.total_attempts} student attempts`}
          icon={Target}
        />
        <StatBox
          label="Telemetry"
          value={String(totals.telemetry_events)}
          helper={`${telemetry.focus_loss_count} focus exits`}
          icon={Activity}
        />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card padding="md">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-serif text-heading text-stone-900">Item Type Mix</h2>
          </div>
                ...
                <div key={row.item_type} className="rounded-lg border border-stone-100 p-3">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-stone-500">{row.total_items} items</span>
                    </div>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-2">
                      <span className="w-10 text-[10px] text-stone-400">Load</span>
                      <div className="flex-1 h-2 rounded-full bg-stone-100 overflow-hidden">
                        <div className={cn("h-full rounded-full", colors.bar)} style={{ width: barW(row.total_items, maxTypeItems) }} />
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="w-10 text-[10px] text-stone-400">Eval</span>
                      <div className="flex-1 h-2 rounded-full bg-stone-100 overflow-hidden">
        ...
        <Card padding="md">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-serif text-heading text-stone-900">AI Pass Buckets</h2>
            </div>
            <div className="flex gap-2">
              <span className="rounded-md bg-violet-50 px-2 py-0.5 text-[10px] font-medium text-violet-600">Parsons</span>
              <span className="rounded-md bg-sky-50 px-2 py-0.5 text-[10px] font-medium text-sky-600">Dropdown</span>
              <span className="rounded-md bg-emerald-50 px-2 py-0.5 text-[10px] font-medium text-emerald-600">Trace</span>
            </div>
          </div>
          <div className="space-y-3">
      ...
      <Card padding="md">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-serif text-heading text-stone-900">Theme Spread</h2>
          <div className="ml-auto flex gap-2">
            <span className="rounded-md bg-brand-50 px-2 py-0.5 text-[10px] font-medium text-brand-700">Total</span>
            <span className="rounded-md bg-emerald-50 px-2 py-0.5 text-[10px] font-medium text-emerald-700">Low-AI</span>
          </div>
        </div>
      ...
      <Card padding="md">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-serif text-heading text-stone-900">Student Telemetry</h2>
        </div>
        ...
        {!telemetry.available ? (
          <div className="rounded-xl border-2 border-dashed border-stone-200 px-6 py-10 text-center">
            ...
            <p className="mt-3 text-sm text-stone-500">
              No student interaction data yet. Complete a few practice attempts to populate this panel.
            </p>
          </div>
        ) : (
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-lg border border-stone-200 p-3">
                  <p className="text-[10px] font-semibold uppercase text-stone-400">Total Events</p>
                  <p className="mt-1 text-xl font-bold text-brand-700">{telemetry.total_events}</p>
                </div>
                <div className="rounded-lg border border-stone-200 p-3">
                  <p className="text-[10px] font-semibold uppercase text-stone-400">Student Attempts</p>
                  <p className="mt-1 text-xl font-bold text-stone-900">{totals.total_attempts}</p>
                  <p className="text-[10px] text-stone-400">{totals.correct_attempts} correct</p>
                </div>
              </div>
              <div className="rounded-lg border border-amber-200 bg-amber-50/50 p-3">
                <h3 className="text-sm font-semibold text-stone-800 mb-2">Integrity Signals</h3>
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-md bg-white border border-amber-100 p-2.5">
                    <p className="text-[10px] text-stone-400">Focus Exits</p>
                    <p className="text-lg font-bold text-amber-700">{telemetry.focus_loss_count}</p>
                  </div>
                  <div className="rounded-md bg-white border border-amber-100 p-2.5">
                    <p className="text-[10px] text-stone-400">Resumes</p>
            ...
            {/* Event breakdown */}
            <div>
              <h3 className="text-sm font-semibold text-stone-800 mb-3">Event Volume</h3>
