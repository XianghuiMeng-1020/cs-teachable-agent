  ...
  return (
    <div className={cn("rounded-xl border border-slate-200 bg-slate-50/80 p-4", className)}>
      ...
      <textarea
        ...
        className="mt-3 w-full rounded border border-slate-300 bg-white p-2 text-sm min-h-[60px]"
        disabled={!taId}
      />
      <Button
      ...
      {(responseA || responseB) && (
        <div className="mt-4 grid grid-cols-2 gap-3">
          <div className="rounded border border-slate-200 bg-white p-2 text-sm">
            <p className="text-xs font-medium text-slate-500 mb-1">Response A</p>
            <p className="text-slate-700 whitespace-pre-wrap">{responseA || "—"}</p>
          </div>
          <div className="rounded border border-slate-200 bg-white p-2 text-sm">
            <p className="text-xs font-medium text-slate-500 mb-1">Response B</p>
            <p className="text-slate-700 whitespace-pre-wrap">{responseB || "—"}</p>
          </div>
        </div>
      )}
    </div>
