...
export function PointsSystem({ points, level, className }: PointsSystemProps) {
  ...
  return (
    <div className={cn("rounded-xl border border-stone-200 bg-gradient-to-br from-amber-50/80 to-orange-50/50 p-4", className)}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-2xl font-bold text-stone-900">{points}</p>
            <p className="text-xs text-stone-500">Points</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Trophy className="h-5 w-5 text-amber-500" />
          <span className="text-lg font-semibold text-stone-800">Level {level}</span>
        </div>
      </div>
