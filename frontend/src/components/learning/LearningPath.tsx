  ...
  return (
    <div className={cn("rounded-xl border border-stone-200 bg-white p-4", className)}>
      ...
      {recommended.length > 0 && (
        <div className="mt-4">
          <p className="text-xs font-medium text-stone-500">Recommended next</p>
          <ul className="mt-1 space-y-1">
            {recommended.slice(0, 3).map((ku) => (
              <li key={ku.id}>
                <Link
                  to={ROUTES.teach}
                  className="flex items-center justify-between rounded-lg px-2 py-1.5 text-sm text-stone-700 hover:bg-stone-50"
                >
