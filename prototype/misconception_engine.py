"""
Misconception engine (Stage C + D): activate, correct (unlearn), and relearn.
- Stage C: activate a misconception for a unit; trace misconception_activation.
- Stage D: apply correction (unlearning), record relearning evidence, transition to learned.
"""
...
def activate_misconception_for_unit(
    ...
    """
    Activate a misconception for the given unit. Updates the state tracker and
    records a misconception_activation trace event.
    Returns True if activation was applied, False if unit unknown or already active.
    """
...
def apply_correction(
    ...
    """
    Apply unlearning: remove misconception from unit, set status to corrected,
    write correction_evidence, record correction_event trace.
...
# Relearning policy: at least one correction + at least one relearning event (teaching or successful_task).
...
def add_relearning_evidence_from_teaching(
    ...
    *,
    ...
    """
    Append relearning_evidence (type=teaching) for the unit, then try transition to learned
    if policy is satisfied. Records relearning_event trace. Returns True if evidence was added.
    """
    if unit_id not in tracker.get_unit_ids():
        return False
...
def add_relearning_evidence_from_successful_task(
    ...
    *,
    ...
    """
    Append relearning_evidence (type=successful_task) for the unit, then try transition to learned
    if policy is satisfied. Records relearning_event trace. Returns True if evidence was added.
    """
    if unit_id not in tracker.get_unit_ids():
        return False
